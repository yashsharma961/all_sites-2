      ...
      if (config.hasOwnProperty("skuSelector")) {
        ...
        for (let i = 0; i < data.length; i++) {
          try {
            ...
            if (obj.hasOwnProperty("url")) val = window.location.href;
      ...
      if (Array.isArray(data.avail)) {
        for (let i = 0; i < data.avail.length; i++) {
          if (div.querySelector(data.avail[i])) {
            if (div.querySelector(data.avail[i]).textContent.includes("deliv"))
              return 2;
            else return 0;
          }
        }
